...
namespace ParamNexusDB
{
    public class ParamWriter
    {
        ...
        public void WriteGameParams()
        {
            ...
            var bnds = new Dictionary<string, BND3>();
            using (var con = GetConnection())
            {
                ...
                using (var cmd = new SQLiteCommand(@"SELECT * FROM 'bnd_metadata'", con))
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        bnds.Add(Path.GetFileName(filename), bnd);
                    }
                }
                ...
                using (var cmd = new SQLiteCommand(@"SELECT * FROM 'bnd_contents'", con))
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        bnds[source_file].Files.Add(file);
                        if (files.ContainsKey(paramType))
                        {
                            var dictValue = files.TryGetValue(paramType, out List<BinderFile> value) ? value : new List<BinderFile>();
                            dictValue.Add(file);
                        }
                        else
                        {
                            var dictValue = new List<BinderFile>
                            {
                                file
                            };
                            files.Add(paramType, dictValue);
                        }
                    }
                }
                ...
                foreach (KeyValuePair<string, List<BinderFile>> entry in files)
                {
                    ...
                    foreach (BinderFile file in entry.Value)
                    {
                        ...
                        using (var metadataCmd = new SQLiteCommand(@"SELECT * FROM param_metadata WHERE ParamType = $ParamType", con))
                        {
                            ...
                            var paramDef = paramTypeToParamDef[entry.Key];
                            var paramFile = new PARAM();
                            paramFile.ParamType = entry.Key;

                            AddParamToCommand(metadataCmd, @"$ParamType", entry.Key);
